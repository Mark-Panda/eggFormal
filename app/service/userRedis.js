    ...
    class userRedisdata extends app.Service {
        ...
        async getRaw(token) {
            ...
            if (userData.version == curVersion) {
                ...
                if (userData.userId < 0) {
                    ...
                    await redis.expire(`u:${userData.userId}`, timeLimit);
                }
        ...
        async addUser(userId, mechanismId) {
            ...
            if (raw) {
                ...
                await redis.set(`u:${raw.userId}`, JSON.stringify(raw.token), 'EX', timeLimit);
            }
            return raw;
        }
        ...
        async delUser(userInfo) {
            try {
                await redis.del(`u:${userInfo.userId}`);
        ...
        async getToken(userId) {
            let token = await redis.get(`u:${userId}`);
            if (token) {
        ...
        async updateUserInfo(tokon) {
            ...
            if (raw) {
                ...
                await redis.set(`u:${raw.userId}`, JSON.stringify(raw.tokon), 'EX', timeLimit);
            }
        }
        // 创建用户数据
